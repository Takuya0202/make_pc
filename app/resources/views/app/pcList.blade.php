            ...
            <div class="w-full">
                @if ($targetPcList)
                    ...
                    @foreach ($targetPcList->parts as $part)
                            <div class="w-full my-5 flex items-center justify-between border-y-2 border-[#d1d5db] bg-white px-3 py-5 ">
                                <p class="m-3 border-r-2 border-[#d1d5db] pr-3"><img src="{{asset(str_starts_with($part->image,'images') ? $part->image : 'storage/' . $part->image)}}" alt="リストに追加された商品画像"
                                    class="w-20 h-20 rounded-xl "></p>
